...
unidirectional and some bidirectional conversions between `LatLng`, `Waypoint`,
`Coordinate`, `Point`, `Bounds`, `Rect`, `Polygon` types.

`Line`, `LineString`, and `MultiLineString` types can be converted to a
`Vec<Waypoint>` for use in a `directions` or `distance_matrix` request fairly
easily. This can be done by iterating over them and converting each resulting
`Point` to a `Waypoint`.
