...
impl<'a> ReverseRequest<'a> {
    ...
    /// preferred language as specified in the `Accept-Language` header, or the
    ...
    /// readable for both the user and locals. To achieve that goal, it returns
    /// street addresses in the local language, transliterated to a script
    /// readable by the user if necessary, observing the preferred language. All
    /// other addresses are returned in the preferred language. Address
    /// components are all returned in the same language, which is chosen from
    /// the first component.
    ///
