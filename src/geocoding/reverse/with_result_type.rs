...
impl ReverseRequest {
    ...
    pub fn with_result_type(&mut self, result_type_element: PlaceType) -> &mut ReverseRequest {
        ...
        match &mut self.result_type {
            ...
            None => self.result_type = Some(vec![result_type_element]),
    ...
    pub fn with_result_types(&mut self, result_type_slice: &[PlaceType]) -> &mut ReverseRequest {
        ...
        match &mut self.result_type {
            // If there are no filters in the request struct, initialize field:
            None => self.result_type = Some(result_type_slice.to_vec()),
            // If there are already filter(s), append to them:
            Some(reverse_request_struct) => for result_type_element in result_type_slice { reverse_request_struct.push(result_type_element.to_owned()) }
        } // match
        // Return modified ReverseRequest struct to caller.
        self
    } // fn
