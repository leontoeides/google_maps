...
//! This will make using these crates together a little less burdensome.
//! Includes some unidirectional and some bidirectional `try_from` conversions
//! between `LatLng`, `Waypoint`, `Coordinate`, `Point`, `Bounds`, `Rect`,
//! `Polygon` types. `Line`, `LineString`, and `MultiLineString` types can be
//! converted to a `Vec<Waypoint>` for use in a `directions` or
//! `distance_matrix` request quite easily. This can be done by iterating over
//! them and converting each resulting `Point` to a `Waypoint`.
//!
